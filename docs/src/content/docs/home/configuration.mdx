...
client.ryanlink = new RyanlinkManager({
  ...
  //playerClass: yourCustomPlayerClass // class yourCustomPlayerClass extends Player {} .. make your own player class and types, the RyanlinkManager is generic so then you can do new RyanlinkManager<yourCustomPlayerClass>({})
  client: {
    // client: client.user
  ...
  playerOptions: {
    ...
    allowCustomSources: false, // set to true if you want to allow custom sources which ryanlink does not support (yet)
    volumeDecrementer: 0.75, // on client 100% == on ryanlink 75%
