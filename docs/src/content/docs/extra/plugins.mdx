...
```typescript
// Get lyrics for the current track (uses player session)
const lyrics = await player.node.lyrics.getCurrent(player.guildId);
const lyrics2 = await player.node.lyrics.getCurrent(player.guildId, true); // skipTrackSource

// Get lyrics for a specific encoded track
const lyrics3 = await player.node.lyrics.get(track, false);

// Subscribe to real-time lyrics lines
...
const lyrics4 = await player.getCurrentLyrics();
const lyrics5 = await player.getLyrics(track);
